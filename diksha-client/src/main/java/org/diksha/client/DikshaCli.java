...
public class DikshaCli {
	public static void main(String[] args) {
		...
		options.addOption(Option.builder("adminit").longOpt("admin-init").desc("shortcut to creating necessary domain, dynamodb tables, config and policies").build());
		options.addOption(Option.builder("admclean").longOpt("admin-clean").desc("HARD deletes of domain, dynamodb tables... policies need to be manually deleted").build());
	...
	private static void init() {
			...
			String [] createSchedulerConfigArgs = {"cf1", "https://swf.us-east-1.amazonaws.com", "dikshaDomain", "70000", "dikshaList"};			
			DyDBUtils.createSchedulerConfig(createSchedulerConfigArgs);
			
			IAMUtils.createSecurityPolicyAdmin("dikshaAdminPolicy");
			IAMUtils.createSecurityPolicyDesigner("dikshaDesignerPolicy");
			IAMUtils.createSecurityPolicyUser("dikshaUserPolicy");
			IAMUtils.createSecurityPolicyWorkflow("dikshaWorkflowPolicy");
